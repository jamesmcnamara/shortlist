...
type Variant =
  "room" | "roomCalm" | "roomSignal" | "roomTable" | "feed" | "ballot";
  ...
  return (
    <main className={styles.page}>
      ...
      <div
        ...
        role="tablist"
        aria-label="Mockup variants"
      >
        <button
          className={variant === "room" ? styles.selectedTab : ""}
          onClick={() => setVariant("room")}
          role="tab"
          aria-selected={variant === "room"}
        >
          Room / Base
        </button>
        <button
          className={variant === "roomCalm" ? styles.selectedTab : ""}
          onClick={() => setVariant("roomCalm")}
          role="tab"
          aria-selected={variant === "roomCalm"}
        >
          Room / Calm
        </button>
        <button
          className={variant === "roomSignal" ? styles.selectedTab : ""}
          onClick={() => setVariant("roomSignal")}
          role="tab"
          aria-selected={variant === "roomSignal"}
        >
          Room / Signal
        </button>
        <button
          className={variant === "roomTable" ? styles.selectedTab : ""}
          onClick={() => setVariant("roomTable")}
          role="tab"
          aria-selected={variant === "roomTable"}
        >
          Room / Table
        </button>
        <button
          className={variant === "feed" ? styles.selectedTab : ""}
          onClick={() => setVariant("feed")}
          role="tab"
          aria-selected={variant === "feed"}
        >
          Feed
        </button>
        <button
          className={variant === "ballot" ? styles.selectedTab : ""}
          onClick={() => setVariant("ballot")}
          role="tab"
          aria-selected={variant === "ballot"}
        >
          Ballot
        </button>
      </div>
  ...
  return (
    <div className={styles.intro}>
      <div>
        <h1>{label}</h1>
        <p>
          7 nominees <span>·</span> 2 votes left this month
        </p>
      </div>
      <button className={styles.primaryButton}>
        Add a nominee <span>+</span>
      </button>
    </div>
  );
}
...
function VoteButton({
  ...
  return (
    <button
      className={`${styles.voteButton} ${compact ? styles.voteCompact : ""}`}
    >
      ...
      <section className={styles.roomStage}>
        <div className={styles.roomPosters}>
          {movies.map((movie, index) => (
            <div
              className={
                index === 0 ? styles.activePoster : styles.secondaryPoster
              }
              key={movie.title}
            >
              <Poster movie={movie} rank={index + 1} />
              <div className={styles.posterMeta}>
                <span>{movie.initials}</span>
                <b>{movie.votes}</b>
              </div>
            </div>
          ))}
        ...
        <p className={styles.pullQuote}>
          “It&apos;s pretty meh, but Liv Tyler!”
        </p>
        <div className={styles.threadSignal}>
          <span>
            <b>3</b> people are in
          </span>
          <span>2 replies</span>
        ...
        <div className={styles.comments}>
          {comments.map(([initials, name, copy]) => (
            <p key={name}>
        ...
        <div className={styles.commentInput}>
          Add to the conversation... <button>↗</button>
        </div>
      </section>
    </>
  );
  ...
  return (
    <>
      ...
      <section className={`${styles.roomStage} ${styles.roomCalm}`}>
        ...
        <div className={styles.threadHeader}>
          <div>
            <span className={styles.kicker}>Current favorite</span>
            <h2>{active.title}</h2>
          </div>
          <VoteButton votes={active.votes} />
        </div>
        <div className={styles.calmQuote}>
          <span className={styles.avatarSmall}>JO</span>
          <div>
            <b>Jackie O</b>
            <p>“It&apos;s pretty meh, but Liv Tyler!”</p>
          </div>
        </div>
        <div className={styles.calmQuote}>
          <span className={`${styles.avatarSmall} ${styles.gold}`}>MS</span>
          <div>
            <b>Meryl Streep</b>
            <p>“I absolutely love the needle drop in Mama Tried.”</p>
          </div>
        </div>
        <div className={styles.commentInput}>
          Add to the conversation... <button>↗</button>
      ...
      <section className={`${styles.roomStage} ${styles.roomSignal}`}>
        ...
        <div className={styles.signalPanel}>
          <div className={styles.signalTop}>
            <span className={styles.kicker}>Live discussion</span>
            <span className={styles.liveDot}>● 3 in the room</span>
          </div>
          <div className={styles.threadHeader}>
            <div>
              <h2>{active.title}</h2>
              <span className={styles.signalMeta}>Nominated by Jackie O</span>
            </div>
            <VoteButton votes={active.votes} />
          </div>
          <p className={styles.pullQuote}>
          ...
          <div className={styles.signalReply}>
            <span className={`${styles.avatarSmall} ${styles.gold}`}>MS</span>
            <span>
              <b>Meryl Streep</b> “The needle drop is perfect.”
            </span>
          </div>
          <div className={styles.signalInput}>
            Write a reply... <button>↗</button>
          </div>
        </div>
      </section>
    </>
  ...
  return (
    <>
      ...
      <div className={styles.filmstrip}>
        {movies.map((movie, index) => (
          <div className={styles.filmItem} key={movie.title}>
            <Poster movie={movie} rank={index + 1} />
            <VoteButton votes={movie.votes} compact />
          </div>
        ))}
      </div>
      ...
      <div className={styles.activity}>
        <Activity initials="JO" accent="blue">
          <b>Jackie O</b> nominated <strong>The Strangers</strong>
          <small>12 min ago</small>
        </Activity>
        <Activity initials="MS" accent="gold">
          <b>Meryl Streep</b> voted for <strong>The Strangers</strong>
          <small>8 min ago</small>
        </Activity>
        <Activity initials="TK" accent="green">
          <b>Theo K</b> replied to <strong>Us</strong>
          <small>Yesterday</small>
        </Activity>
      </div>
      <button className={styles.wideButton}>
        Open all discussions <span>→</span>
      </button>
    </>
  );
}
...
function Activity({
  ...
  return (
    <article className={styles.activityItem}>
      <span className={`${styles.avatarSmall} ${styles[accent]}`}>
        {initials}
      </span>
      <p>{children}</p>
    </article>
  ...
  return (
    <>
      ...
      <div className={styles.ballotTop}>
        <span className={styles.kicker}>The group ballot</span>
        <h1>What gets watched next?</h1>
        <p>Choose one. Your vote is private until you cast it.</p>
      </div>
      <div className={styles.ballotList}>
        {movies.map((movie, index) => (
          <article className={styles.ballotRow} key={movie.title}>
            <span className={styles.ballotNumber}>0{index + 1}</span>
            <Poster movie={movie} rank={index + 1} />
            <div className={styles.ballotCopy}>
              <h2>{movie.title}</h2>
              <p>
                {movie.year} <span>·</span> nominated by {movie.nominator}
              </p>
              <div className={styles.voteMeter}>
                <span
                  style={{ width: `${movie.votes ? movie.votes * 24 : 8}%` }}
                />
              </div>
              <small>{movie.votes} votes so far</small>
            </div>
            <button
      ...
      <div className={styles.ballotFooter}>
        <span>Votes reset on the first of the month.</span>
        <button className={styles.primaryButton}>
          Cast your vote <span>→</span>
        </button>
      </div>
    </>
  );
