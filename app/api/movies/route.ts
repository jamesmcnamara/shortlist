...
async function withDB(handler: (db: DB) => Promise<Response>) {
  ...
  try {
    ...
    return json(
      {
        error:
          "The database request failed. Check your DATABASE_URL and migration.",
      },
      500,
    );
  }
  ...
  return withDB(async (db) => {
    ...
    const year =
      body?.year === undefined || body?.year === null || body?.year === ""
        ...
        : Number(body.year);
    const tmdbId =
      body?.tmdbId === undefined || body?.tmdbId === null || body?.tmdbId === ""
        ? null
        : Number(body.tmdbId);
    const posterUrl =
    ...
    const tmdbRating =
      body?.tmdbRating === undefined ||
      body?.tmdbRating === null ||
      body?.tmdbRating === ""
        ? null
    ...
    if (
      ...
      return json(
        { error: "Enter a movie title and an optional valid year." },
        400,
      );
    }
    ...
    const [movie] = await db
      .insert(movies)
      .values({ title, year, tmdbId, posterUrl, description, tmdbRating })
      .returning();
